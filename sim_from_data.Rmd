...
hazzest_t2d <- basehaz(cox_t2d, predict_data, centered = TRUE)
hazzest_death <- basehaz(cox_death, predict_data)
...
gridExtra::grid.arrange(
  ggplot(data = data.frame(x = hazzest_death$time, y = exp(-hazzest_death[,1])))+
    geom_line(aes(x = x, y = y), color = "red") +
    geom_function(fun = true_surv_death_func1),
  ggplot(data = data.frame(x = hazzest_death$time, y = exp(-hazzest_death[,2])))+
    geom_line(aes(x = x, y = y), color = "red") +
    geom_function(fun = true_surv_death_func2),
  ggplot(data = data.frame(x = hazzest_t2d$time, y = exp(-hazzest_t2d[,1])))+
    geom_line(aes(x = x, y = y), color = "red") +
...
simEventDataCox <- function(N, 
  ...
  while(length(alive) != 0){
    ...
    cumInt_list <- lapply(cox_fits, function(model) basehaz(model, sim_data[alive,]))
    ...
    for(j in 1:num_events){
      event_times[sim_data[alive, (2+j)] == n_event_max[j], j] <- Inf
    }
    ...
    for(i in 1:num_alive){
      sim_data[alive[i], (Deltas[i] + 2)] <- sim_data[alive[i], (Deltas[i] + 2)] + 1
    }
    ...
    T_k <- T_k[alive]                           
    cum_int_Tk <- cum_int_Tk[alive,]
    event_times <- event_times[alive,]
  }
...
new_data <- simEventDataCox(1000, cox_fits, L0_old = data$L0)
```
...
```{r}
# Transform data into tstart tstop format
data_int2 <- IntFormatData(new_data, N_cols = 6)
