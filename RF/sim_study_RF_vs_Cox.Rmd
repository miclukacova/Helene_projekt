...
list_old_vars <- list(age = follic$age, hgb = follic$hgb, clinstg = follic$clinstg, 
                      ch = follic$ch)
true_data <- simEventObj(N = 10^6, RF_fit, list_old_vars = list_old_vars)
```
...
We then simulate 100 normal sized data sets using `simEventObj`. For each new data set we fit a random forest and cox proportional hazard models. 
...
for(i in 1:100){
  ...
  new_data[[i]] <- simEventObj(N = N, RF_fit, list_old_vars = list_old_vars)
  # RF fit
...
true_data0 <- simEventObj(N = 10^6, RF_fit, list_old_vars = list_old_vars0)
true_data1 <- simEventObj(N = 10^6, RF_fit, list_old_vars = list_old_vars1)
```
