...
Plot the true CHF together with the 10 estimated CHF´s
...
plot(time_points, true_chf12, type="l",
     xlab="Time", ylab="CHF",
     col=2, lty=1, lwd=2, xlim=c(0,10), ylim=c(0, 2))
for (i in seq_along(y.pred)) {
...
t_prime <- 2
data <- simEventTV(4000, beta = beta, term_deltas = c(0,1), tv_eff = beta_TV, t_prime = t_prime)
```
...
plot(time_points,y.pred$chf[1,,1], type="l", xlab="Time",
     ylab="Process 1 CHF", col=1, lty=1, lwd=2, xlim = c(0,5), ylim = c(0,1))
...
plot(time_points,y.pred$chf[1,,2], type="l", xlab="Time",
     ylab="Process 2 CHF", col=1, lty=1, lwd=2, xlim = c(0,5), ylim = c(0,1))
