...
```{r}
profvis::profvis({
simEventDataTry2 <- function(N,                      # Number of individuals
                            beta = NULL,            # Effects
...
simEventDataTry2(10^5)
})
```

Virker egentlig ok hurtig, tiden bliver brugt i cpp funktionerne, men jeg ved ikke om det giver mening at optimere derinde, eller om man overhovedet kan?

```{r}
my_bench <- bench::mark(
      "timeVar" = simEventDataTry(2*10^3, eta = c(0.1,0.2,0.1,0.2)),
      "gammel" = simEventData(2*10^3, eta = c(0.1,0.2,0.1,0.2)),
      check = FALSE)
