...
$L$ is an indicator for a change in the covariate process.  
...
data1 <- sim_event_data(N = N, beta = beta, eta = eta, nu = nu)
plot_event_data(data1) 

data2 <- sim_event_data(N = N, beta = beta[,2:3], eta = eta[2:3], nu = nu[2:3], at_risk = at_risk1, 
        term_deltas = c(0,1))
plot_event_data(data2)

data3 <- sim_event_data(N = N, beta = beta, eta = eta, nu = nu, at_risk = at_risk1, 
        term_deltas = c(0,1,2,3))
plot_event_data(data3)
```
...
# Generating data
N <- 2000
data_test <- sim_event_data(N = N, beta = beta, eta = eta, nu = nu, at_risk = at_risk2, term_deltas = term_deltas)
...
# Survival setting
N <- 1000
data <- sim_surv_data(N = N, beta = matrix(2, ncol = 2, nrow = 2), eta = rep(0.1,2), nu = rep(1.1,2))

ggplot(data) +
  geom_point(aes(x = L0, y = Time)) +
  facet_grid(A ~ Delta, labeller = labeller(A = label_both, Delta = label_both))

survfit_death <- coxph(Surv(Time, Delta == 2) ~ L0 + A, data = data)
confint(survfit_death);

survfit_cens <- coxph(Surv(Time, Delta == 1) ~ L0 + A, data = data)
...
# Operation setting
N <- 50
data1 <- sim_event_data(N = N, beta = beta, eta = eta, nu = nu, at_risk = at_risk2, 
