...
# Kaplan-Meier estimates of survival for scenario E

We let the effect of $L0$ on death vary, and estimate the survival probability with KM. 
...
res5 <- compare_effects(estimator = estimator2, N = N, beta_L_D = 0.5, 
                        eta = c(0.1,0.3,0.1,0.1), nu = c(1.1,1.3,1.1,1.1),
                        beta_L0_L = 2, beta_L0_D = seq(0,1,by = 0.1), beta_A0_D = 0)
```
...
plot_compare(res5, diff_betas= seq(0,1,by = 0.1))+
      ...
      xlab("Effect of L0 on Death")+
      labs(title = "2 year survival probability since T2D")
```

There is a slight difference between the curves. 
