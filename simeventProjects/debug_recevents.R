...
N <- 10^5
################################################################################
# simEventCox works fine on the whole data, and also on the split data
################################################################################
...
predict2.simevent <- function(obj, sim_data){
  ...
  # Chf
  # Each entry is a matrix where each row is the chf of individual j
...
cox_death2 <- survival::coxph(survival::Surv(Time, Delta == 2) ~ L0 + A0, data = new_data)
cox_Disease2 <- survival::coxph(survival::Surv(Time, Delta == 3) ~ L0 + A0, data = new_data)
...
ggarrange(plotEventData(new_data[Delta != 0][1:1000,], title = "Sim Data"),
          plotEventData(data_obs[1:1000,], title = "Original Data"), ncol = 2)

new_data[Delta != 0, table(Delta)]/nrow(new_data)
data_obs[, table(Delta)]/nrow(data_obs)
new_data[Delta != 0, summary(Time)]
data_obs[, summary(Time)]
...
for(i in 1:B){
  ...
  old_vars <- data_obs[, c("L0", "A0")]
  new_data <- simEventObj(N, obj = cox_fits, old_vars = old_vars, event_names = c("C", "D", "L"))
  max_times1[i] <- new_data[Delta != 0, max(Time)]
  max_times2[i] <- data_obs[,max(Time)]
}
...
# Fit some Cox models
cox_cens <- survival::coxph(survival::Surv(Time, Delta == 0) ~ L0 + A0, data = data1)
cox_death <- survival::coxph(survival::Surv(Time, Delta == 1) ~ L0 + A0, data = data1)
cox_Disease <- survival::coxph(survival::Surv(Time, Delta == 2) ~ L0 + A0, data = data1)
...
class(cox_fits) <- "simevent"
old_vars <- data2[, c("L0", "A0", "oldTime")]
new_data <- simEventObj(N = nrow(data2), obj = cox_fits, old_vars = old_vars, event_names = c("C", "D"),
                        useOldVars = TRUE)

# Fit new Cox models
cox_cens2 <- survival::coxph(survival::Surv(Time, Delta == 1) ~ L0 + A0 + oldTime, data = new_data)
cox_death2 <- survival::coxph(survival::Surv(Time, Delta == 2) ~ L0 + A0 + oldTime, data = new_data)
