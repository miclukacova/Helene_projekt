...
We simulate data where the effect of the drug on the development of T2D is respectively 0, -0.5, -1. And the effect of the drug on Death is -0.1. 

```{r}
data0 <- sim_data_setting2(N = N, beta_L_D = 1, beta_A0_D = -0.1,
                           beta_L0_L = 1, beta_A0_L = 0)
data0.5 <- sim_data_setting2(N = N, beta_L_D = 1, beta_A0_D = -0.1,
                           beta_L0_L = 1, beta_A0_L = -0.5)
data1 <- sim_data_setting2(N = N, beta_L_D = 1, beta_A0_D = -0.1,
...
```{r}
# T2D events
T2D_events0 <- data0[Delta == 3]; T2D_events0.5 <- data0.5[Delta == 3]; T2D_events1 <- data0.5[Delta == 3]
...
Apparently both groups have same average baseline covariate, irrespective of the effect of the drug on T2D. But the larger the effect of the drug the later the patients in the Treatment group develop T2D. The number of patients developing T2D is equal for both groups when there is no effect of the drug. but fewer and fewer patients in the Treatment arm develop T2D as the effect of the drug increases due to censoring and death. 
...
T2D_peeps1 <- data0[ID %in% T2D_events1$ID]

# Proportion of placebo and treatment T2D people who have died before Time = 10 
nrow(T2D_peeps0[Time < 10 & Delta == 1 & A0 == 1]) / length(unique(T2D_peeps0[A0 == 1]$ID))
nrow(T2D_peeps0[Time < 10 & Delta == 1 & A0 == 0]) / length(unique(T2D_peeps0[A0 == 0]$ID))

nrow(T2D_peeps0.5[Time < 10 & Delta == 1 & A0 == 1]) / length(unique(T2D_peeps0.5[A0 == 1]$ID))
nrow(T2D_peeps0.5[Time < 10 & Delta == 1 & A0 == 0]) / length(unique(T2D_peeps0.5[A0 == 0]$ID))

nrow(T2D_peeps0[Time < 10 & Delta == 1 & A0 == 1]) / length(unique(T2D_peeps0[A0 == 1]$ID))
nrow(T2D_peeps0[Time < 10 & Delta == 1 & A0 == 0]) / length(unique(T2D_peeps0[A0 == 0]$ID))
```

The porportions are the same!

**skrue på L0 coefficentens effekt på død**
To investigate why the  two groups have the same average baseline covariate we change the effect that L0 has on Death to $0.3$, and investigate data in the same way as before. 

```{r}
data0 <- sim_data_setting2(N = N, beta_L_D = 1, beta_A0_D = -0.1,
                           beta_L0_L = 1, beta_A0_L = 0, beta_L0_D = 0.3)
data0.5 <- sim_data_setting2(N = N, beta_L_D = 1, beta_A0_D = -0.1,
                           beta_L0_L = 1, beta_A0_L = -0.5, beta_L0_D = 0.3)
data1 <- sim_data_setting2(N = N, beta_L_D = 1, beta_A0_D = -0.1,
                           beta_L0_L = 1, beta_A0_L = -1, beta_L0_D = 0.3)
```
