...
$$\lambda_{0}(x,t)=\eta_x \nu_x t^{\nu_x-1} $$
And
$$\phi(x,t) = \exp (\beta_{L_0, x} L_0 + \beta_{L_1, x} L_1 + \beta_{A_0, x} A_0 + \beta_{L, x} L)$$
...
```{r}
res1 <- compare_effects(estimator = estimator1, N = 2000, beta_L_D = seq(0,1,by = 0.1))
plot_compare(res1) +
...
We now vary the effect of the drug on Death, and see how this affects the T2D probbaility.  

```{r}
res2 <- compare_effects(estimator = estimator1, N = 2000, beta_A0_D = seq(-1,0,by = 0.1))
plot_compare(res2, plot_no = 1, diff_betas <- seq(-1,0,by = 0.1)) +
      ylab("Probability of T2D after 5 years")+
      xlab("Effect of Drug on Death")
```
When the Drug has no effect on the intensity T2D but an effect on Death, the probability of getting T2D is larger in the Treatment arm (because less people die). As the effect of the drug on T2D increases the probability of getting T2D gets smaller and smaller for the people in the Treatment arm. 
...
$$ P(U > 3, A_0 = 1, L_0 = 0.5, L_1 = 0) \quad , \quad P(U > 3, A_0 = 1, L_0 = 0.5, L_1 = 0)$$

We estimate the survival probabilities by simulating data as previously. For each data set we estimate the survival function with the Kaplan Meyer estimator. For each fit we predict the survival probability 3 years after debut of T2D for a observation with $L_0 = 0.5$, $L1 = 0$ and $A_0 = 0$ and $A_0 = 1$. That is a person on initial treatment, and a person not on initial treatment. 
...
```
We see as the effect of T2D on Death increases the probability of survival decreases. It does so equally for both treatment and placebo group. This makes sense as the positive effect of the drug on Death is only a consequence of the later T2D diagnose. After the diagnose, both groups stand equally well. Hmm... what about the difference in age between the two groups?
...
# Find age difference between the two groups after T2D diagnose
...
Apparently both groups have same average baseline covariate, but as the effect of the drug increases, they get older in the Treatment group. 
