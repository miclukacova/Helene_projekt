...
```{r}
data0_t <- trans_int_data(data0)
data0_t[, at_risk_oper := as.numeric(A == 0)]

fit0 <- flexsurv::flexsurvreg(formula = Surv(Time, Delta == 2), 
            data = data0_t[at_risk_oper == 1], 
            dist = "weibull")
```
...
```{r}
fit0$coefficients
nu0 <- fit0$coefficients[1]
eta0 <- 1/(fit0$coefficients[2]^nu0)
```
...
data_under_int <- sim_data_setting1(N = N, cens = 0,
                                    ...
                                    nu = c(rep(1.1,3), nu0),
                                    beta_L0_A = fit0$coefficients[3],
                                    beta_L_A = fit0$coefficients[4])
