...
sur_sim <- function(N,                      # Number of individuals
  ...
  # Simulation
  res <- data.frame(ID = numeric(), L0 = numeric(), Time = numeric(), Delta = numeric())
...
eta <- c(0.1, 0.1, 0.1)
nu <- c(1.1, 1.1, 1.1)
beta <- matrix(c(3, 0.1, 3, 0.1, 3,0.1), ncol = 3)
...
library(survival)
survfit <- coxph(Surv(Time, Delta == 0) ~ L0, data = data)
summary(survfit)
