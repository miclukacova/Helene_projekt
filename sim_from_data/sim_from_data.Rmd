...
```{r}
set.seed(373)
data <- simT2D(N = 10000, beta_L0_L = 1, beta_L0_D = 1, beta_L_D = 0.5)
```
...
true_surv_t2d_func <- function(t) exp( - 0.1  * t^(1.1)  * exp(sum(predict_data[1,] * c(1,0,0))))
...
```{r}
set.seed(373)
beta <- matrix(rnorm(2*3), nrow = 2)
data <- simCRdata(50000, beta = beta)
predict_data <- data.frame("L0" = 0.5, "A0" = 0)
...
cox_fits <- list(death = cox_death, t2d = cox_t2d)
new_data <- simEventDataCox(3000, cox_fits, L0_old = data$L0, term_events = c(1,2), 
                            A0_old = data$A0)
...
## Trying with complex data with effects

```{r}
set.seed(375)
beta <- matrix(rnorm(3*5), nrow = 5)
beta[3,1] <- beta[4,2] <- beta[5,3] <- 0
data <- simEventData(50000, beta = beta, term_deltas = c(0,1))
predict_data <- data.frame("L0" = 0.5, "A0" = 0, "N0" = 0, "N1" = 0, "N2" = 0)
true_surv_death_func <- function(t) exp( - 0.1  * t^(1.1)*exp(sum(beta[,2] * predict_data)))
true_surv_t2d_func <- function(t) exp( - 0.1  * t^(1.1)*exp(sum(beta[,3] * predict_data)))

# Fit Cox models
data <- IntFormatData(data, N_cols = 6:8)
cox_death <- coxph(Surv(tstart, tstop, Delta == 1) ~ L0 + A0 + N2, data = data, timefix=FALSE)
cox_t2d <- coxph(Surv(tstart, tstop, Delta == 2) ~ L0 + A0, data = data, timefix=FALSE)
surv_death <- survfit(cox_death, newdata = predict_data, method = "breslow")
surv_t2d <- survfit(cox_t2d, newdata = predict_data, method = "breslow")

cox_fits <- list(death = cox_death, t2d = cox_t2d)
new_data <- simEventDataCox(2000, cox_fits, L0_old = data$L0, term_events = c(1), 
                            A0_old = data$A0, proc_names = c("D", "N2"), n_event_max = c(1, Inf))
new_data <- IntFormatData(new_data, N_cols = 6:7)

# Fit Cox models
cox_death2 <- coxph(Surv(tstart, tstop, Delta == 1) ~ L0 + A0 + N2, data = new_data)
cox_t2d2 <- coxph(Surv(tstart, tstop, Delta == 2) ~ L0 + A0, data = new_data)
